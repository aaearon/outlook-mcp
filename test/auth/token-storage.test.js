...
describe('TokenStorage', () => {
  ...
  describe('_saveTokensToFile', () => {
    ...
    it('should throw error if fs.writeFile fails', async () => {
      ...
      await expect(tokenStorage._saveTokensToFile()).rejects.toThrow(
        writeError
      );
    });
  });
  ...
  describe('isTokenExpired', () => {
    ...
    it('should return true if token is past expiration time (considering buffer)', () => {
      tokenStorage.tokens = {
        expires_at: Date.now() - (baseConfig.refreshTokenBuffer + 1000),
    ...
    it('should return false if token is not expired and outside buffer', () => {
      tokenStorage.tokens = {
        expires_at: Date.now() + (baseConfig.refreshTokenBuffer + 60000),
  ...
  describe('exchangeCodeForTokens', () => {
    ...
    it('should successfully exchange code for tokens and save them', async () => {
      ...
      const requestBody = querystring.parse(
        mockHttpsRequest.write.mock.calls[0][0]
      );
    ...
    it('should reject if saving exchanged token fails', async () => {
      ...
      expect(tokenStorage.tokens.access_token).toBe(
        mockSuccessfulTokenResponse.access_token
      );
    });
    ...
    it('should reject if client ID or secret is missing', async () => {
      ...
      await expect(
        tokenStorage.exchangeCodeForTokens(mockAuthCode)
      ).rejects.toThrow(
  ...
  describe('refreshAccessToken', () => {
    ...
    it('should reject if saving refreshed token fails', async () => {
      ...
      expect(tokenStorage.tokens.access_token).toBe(
        mockSuccessfulRefreshResponse.access_token
      );
    });
    ...
    it('should update refresh_token if a new one is in response', async () => {
      ...
      mockHttpsRequest.callback(mockRes);
      await refreshPromise;
      expect(tokenStorage.tokens.refresh_token).toBe(
        'new_returned_refresh_token'
      );
    });
    ...
    it('should handle concurrent refresh calls by returning the same promise', async () => {
      ...
      // Note: Due to implementation returning .then() wrapper, these may be different
      // promise objects but should resolve to the same value with only one HTTP call.
  ...
  describe('getValidAccessToken', () => {
    beforeEach(() => {
      ...
      jest
        ...
        .mockImplementation(async () => tokenStorage.tokens);
    });
    ...
    it('should propagate error if saving nulled token fails after refresh failure', async () => {
      ...
      jest
        .spyOn(tokenStorage, '_saveTokensToFile')
        .mockRejectedValueOnce(saveError); // This is key

      await expect(tokenStorage.getValidAccessToken()).rejects.toThrow(
        saveError
      );
    ...
    it('should propagate error if saving nulled token fails (no refresh token path)', async () => {
      ...
      const saveError = new Error(
        'Disk write error during null save (no-refresh path)'
      );
      jest
        .spyOn(tokenStorage, '_saveTokensToFile')
        .mockRejectedValueOnce(saveError);
